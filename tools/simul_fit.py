...
def update_texts(f_name, target, new):
    with open(f_name, 'r') as f:
        for line in f.readlines():
            try:
                fargs = map(str, line.strip().strip('\n').split())
                if fargs[0] == target:
                    old = line
            except:
                '''
...
def simul_fit(sample, path, patch, xs_list, cut = ''):
    ...
    VP_D1_2420 = 1.
    if sample > 4316:
        with open(xs_D1_2420) as f:
            for line in f.readlines():
                try:
                    fargs = map(float, line.strip().strip('\n').split())
                    if int(fargs[0]) == sample: ecms, lum, br, eff_D1_2420, ISR_D1_2420, VP_D1_2420, N0_D1_2420 = fargs[1], fargs[2], fargs[3], fargs[7], fargs[8], fargs[9], fargs[10]
                except:
                    '''
                    '''
    ...
    ISR_psipp = 1.
    VP_psipp = 1.
    with open(xs_psipp) as f:
        for line in f.readlines():
            try:
                fargs = map(float, line.strip().strip('\n').split())
                if int(fargs[0]) == sample: ecms, lum, br, eff_psipp, ISR_psipp, VP_psipp, N0_psipp = fargs[1], fargs[2], fargs[3], fargs[7], fargs[8], fargs[9], fargs[10]
            except:
                '''
                '''
    ...
    if not os.path.exists('./figs/'):
        os.makedirs('./figs/')
    canvas_name = './figs/simul_fit_' + str(sample) + '_' + patch + '.pdf'
